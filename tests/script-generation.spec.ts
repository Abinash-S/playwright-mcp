...
test('script generation basic flow', async ({ client, server }) => {
  ...
  // Extract refs from snapshot (simplified for test)
  const snapshotText = snapshotResult.content[0].text;
  const nameInputMatch = snapshotText.match(/textbox "Enter name": <input[^>]*data-pw-testid="([^"]*)"[^>]*>/);
  const submitButtonMatch = snapshotText.match(/button "Submit": <button[^>]*data-pw-testid="([^"]*)"[^>]*>/);
  
  if (!nameInputMatch || !submitButtonMatch) {
    throw new Error('Could not find element refs in snapshot');
  }
...
test('script generation with retry filtering', async ({ client, server }) => {
  ...
  const snapshotText = snapshotResult.content[0].text;
  const buttonMatch = snapshotText.match(/button "Click me": <button[^>]*data-pw-testid="([^"]*)"[^>]*>/);
  
  if (!buttonMatch) {
    throw new Error('Could not find button ref in snapshot');
  }
